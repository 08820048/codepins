...
- **行号槽区域**：点击编辑器左侧行号区域的图钉图标添加/移除图钉
- **选择文本**：选中代码后，点击出现的浮动操作按钮添加为图钉
- **注释标记**：在代码中添加 `@pin` 或 `@pin: 备注内容` 格式的注释自动创建图钉
...
#### 🎉 新功能
* **📌 行号槽区域图钉**：点击编辑器左侧行号区域的图钉图标可以快速添加/移除图钉
* **👆 选择文本浮动按钮**：选中代码后自动显示添加图钉的浮动按钮
* **💬 注释标记自动识别**：支持通过 `@pin` 或 `@pin: 备注内容` 格式的注释自动创建图钉

#### 🔧 改进
* **用户体验**：提供更多直观、便捷的图钉添加方式
* **团队协作**：通过注释标记功能增强团队协作体验
* **界面优化**：新增浮动按钮和行号区图标，提升可见性
